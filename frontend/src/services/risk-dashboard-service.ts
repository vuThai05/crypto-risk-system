...
function toMarketAsset(item: RiskyAsset): MarketOverviewAsset {
  return {
    ...
    price_usd: item.snapshot?.price_usd ?? null,
    change_1h_avg: item.snapshot?.percent_change_1h ?? null,
    change_24h_avg: item.snapshot?.percent_change_24h ?? null,
    market_cap: item.snapshot?.market_cap ?? null,
    volume_24h: item.snapshot?.volume_24h ?? null,
